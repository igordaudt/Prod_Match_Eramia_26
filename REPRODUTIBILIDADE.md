...
| `data/ground_truth/correlations-295-different-948.tsv` | As 948 correlações produto↔item reais | Subproduto do uso real de uma plataforma de orçamentação; é o *ground truth* real em si |
...
### Caso à parte: o artigo (`paper/`, `paper_v2/`)

Não se encaixa nos três grupos acima: é essencial e não regenerável (é o texto em si), mas **não é versionado**, por uma razão que não tem a ver com tamanho ou regenerabilidade — a submissão está em revisão cega, e manter o artigo no repositório remoto (nominal, sob o usuário real do autor) comprometeria o anonimato. `paper_v2/` (versão atual, anonimizada) e `paper/` (versão anterior) ficam só no disco local, fora do `.gitignore` por decisão explícita. Ver nota no `README.md`.
...
- `results/real_295_bge_m3_puro/sinapi_llm_metrics.csv`, `sinapi_llm_precision_recall_interpolated.png` (5º método na Tabela 1 — BGE-M3 denso puro sobre o conjunto real)
...
| `results/real_295_bge_m3_puro/sinapi_llm_candidates.jsonl` | 33 MB | `retrieval_sinapi.py --methods bge_m3 ...` (ver README) | Mesmo motivo do equivalente em `real_295` |
| `results/real_295_bge_m3_puro/sinapi_llm_retrieval_results.json` | 17 MB | idem | idem |
...
Nenhum desses arquivos, se apagado, causa perda de informação: todos são recriáveis a partir do código em `scripts/` aplicado sobre os dados do grupo 1, na ordem descrita no `README.md` (`prepare_retrieval_data.py` → `run_retrieval.py`/`run_retrieval_ppc_weak.py`/`retrieval_sinapi.py --methods bge_m3` → `build_combined_dataset.py` → `eda_corpus.py`/`eda_bertopic.py`/`categ_map_analysis.py` → `jaccard_median_split.py` → `jaccard_split_precision_recall.py`/`jaccard_split_ndcg.py` → `merge_datasets_analysis.py`).

## Resumo em números

| Grupo | Tamanho aproximado |
|---|---|
| 1 + 2 (o que versionar) | ~6,5 MB (`git ls-files` somado) |
| Total atual do diretório em disco | ~366 MB |

A diferença entre os dois números inclui o grupo 3 (caches e saídas brutas, regeneráveis) e também material mantido **só localmente**, fora do `.gitignore` por decisão explícita e não por tamanho: `paper/` e `paper_v2/` (o artigo, nas duas versões — fora do remoto por anonimato, ver acima) e o estudo exploratório em `results/real_295_v3_no_trivial/` — nenhum dos três está no repositório remoto.
