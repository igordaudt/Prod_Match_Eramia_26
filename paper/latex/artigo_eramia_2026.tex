...
\textbf{M\'etodos.} Foram avaliados quatro m\'etodos, implementados localmente: BM25 \cite{robertson:09}; E5 \emph{multilingual} denso \cite{wang:22} (\texttt{multilingual-e5-small}); E5 \emph{hybrid} (E5~+~BM25 via \emph{Reciprocal Rank Fusion} \cite{cormack:09}); e BGE-M3 \emph{hybrid} \cite{chen:24} (\texttt{bge-m3}~+~BM25 via RRF). Cada consulta retornou at\'e 100 candidatos, em CPU, sem GPU. As m\'etricas (MAP e NDCG@10/@100) s\~ao calculadas por consulta e macro-agregadas, invariantes \`a escala absoluta dos scores. Calculou-se tamb\'em a similaridade de Jaccard entre cada consulta e a descri\c{c}\~ao do item-alvo, nos dois conjuntos.
