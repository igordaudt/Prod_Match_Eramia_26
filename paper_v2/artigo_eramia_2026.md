...
> **Nota sobre BGE-M3 denso puro:** avaliado nos dois conjuntos — no sintético controlado (Seção 4.3) obtém o maior MAP entre cinco métodos; no conjunto real (Seção 4.2), reaproveitando o mesmo cache de embeddings do BGE-M3 hybrid, resultados em `results/real_295_bge_m3_puro/`.

### 3.5 Protocolo Experimental

Cada consulta retornou *limit* = 100 candidatos. Os quatro métodos principais (Seção 3.4) foram avaliados nas mesmas implementações locais, em uma única rodada por conjunto, com o cache de embeddings do BGE-M3 pré-computado e validado contra cada corpus antes do início da rodada; o BGE-M3 denso puro foi avaliado à parte, reaproveitando o mesmo cache.
...
| BGE-M3 (denso) | 0,7568 | 0,2023 | 0,1062 | 0,9624 | 0,9930 | 0,9994 | 297,1 |
...
Dados completos: `results/real_295/sinapi_llm_metrics.csv`, `results/real_295/sinapi_llm_retrieval_results.json`. Curvas Precisão×Recall (bruta e interpolada): `results/real_295/sinapi_llm_precision_recall*.png`. BGE-M3 (denso): `results/real_295_bge_m3_puro/`.
...
**BGE-M3 denso puro vs. hybrid no conjunto real.** Isolando o efeito da fusão híbrida com BM25: o BGE-M3 denso puro obtém MAP = 0,7568, quarto colocado entre cinco métodos (acima do E5 denso, abaixo do E5 hybrid). A fusão via RRF com BM25 adiciona **+0,0365 de MAP** (0,7568 → 0,7932) ao custo de ≈27% mais latência (297,1 ms → 378,6 ms) — no conjunto real, a fusão híbrida compensa claramente seu custo computacional.
...
|---|---|---|
| BM25 | 0,559 (pior de 5 métodos) | **0,7967** (melhor de 5 métodos) |
| E5 (denso) | 0,580 | 0,7116 |
| E5 hybrid | 0,675 | 0,7638 |
| BGE-M3 (denso) | **0,723** (melhor de 5 métodos) | 0,7568 |
| BGE-M3 hybrid | 0,711 | 0,7932 |

A hierarquia de métodos se **inverte** entre os dois conjuntos, e a fusão híbrida com BM25 tem efeito **oposto** nos dois: ajuda no conjunto real (BGE-M3 denso 0,7568 → hybrid 0,7932, +0,0365 MAP) e atrapalha levemente no sintético (0,723 → 0,711, −0,012 MAP) — a fusão com BM25 só compensa quando o próprio BM25 tem algo a contribuir, o que depende do perfil lexical dos dados. No sintético controlado — consultas construídas por variação de notação técnica com Jaccard deliberadamente baixo — o BM25 é o método mais fraco (MAP 0,559) e o BGE-M3 (denso ou hybrid) lidera. No conjunto real, essa ordem se inverte quase por completo: o BM25 passa a ser o método de maior MAP (0,797), o BGE-M3 denso puro cai para quarto lugar (0,757), e o E5 denso permanece o pior desempenho entre os cinco métodos (0,712).
...
(BGE-M3 denso puro: MAP = 0,723 no conjunto sintético — o maior valor entre os cinco métodos nesse conjunto — e 0,757 no conjunto real; ver Seção 4.2.)
...
Uma rodada adicional isolou o efeito específico da fusão híbrida: o **BGE-M3 denso puro**, agora avaliado nos dois conjuntos (Seção 4.2), obtém MAP = 0,757 no conjunto real (quarto entre cinco métodos) e MAP = 0,723 no sintético (o melhor entre cinco). A fusão com BM25 via RRF tem efeito **oposto** nos dois conjuntos: adiciona +0,037 de MAP no conjunto real, mas subtrai 0,012 no sintético — evidência direta de que o ganho da fusão híbrida depende de o BM25, isoladamente, ter algo de útil a contribuir, o que por sua vez depende do perfil lexical dos dados.
...
**Trabalhos futuros:** (i) validação estatística formal (ex. bootstrap pareado) da diferença de vantagem semântica entre as metades de Jaccard da Seção 4.5, hoje reportada como diferença de médias sem teste de significância; (ii) investigar roteamento de retriever por categoria de item e por tipo de variação terminológica esperada, dado o padrão heterogêneo encontrado nas Seções 4.3–4.4; (iii) ampliação do conjunto sintético controlado para cobrir outras classes de variação (normas NBR×ASTM, classes de pressão, bitolas comerciais) e para produzir uma metade de Jaccard superior com composição mais equilibrada entre real e sintético; (iv) ampliação do ground truth real conforme novas correlações forem validadas em produção.
